...
(function registerCaseDetailsModal(app) {
  ...
  const formatDateTime = app.formatDateTime;
  ...
  function DetailBlock({ label, value, emphasis }) {
    return (
      <div className="rounded-2xl bg-slate-50 p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
          {label}
        </p>
        <p className={`${emphasis ? "text-lg font-bold text-ink" : "text-sm font-medium text-slate-700"} mt-2`}>
          {value}
        </p>
      </div>
    );
          ...
          <div className="flex items-start justify-between gap-4 border-b border-slate-100 bg-white/95 px-6 py-5 backdrop-blur">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3">
                ...
                <span
                  className={`rounded-full px-3 py-1 text-xs font-bold ${badgeStyles[displayStatus] || badgeStyles.New}`}
                >
                  {displayStatus}
                </span>
              </div>
          ...
          <div className="grid flex-1 gap-0 overflow-hidden lg:grid-cols-[1.1fr,0.9fr]">
            <div className="overflow-y-auto p-6">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <DetailBlock
                  label="Date Reported"
                  value={caseItem.dateReported ? formatDate(caseItem.dateReported) : "Not reported yet"}
                />
                <DetailBlock
                  label="Date Created"
